...
1. A simple forward contract pricing [example](#forwardExample) demonstrates that **arbitrage dominates expected value** in pricing, and they might differ. 
    - Risk-free arbitrage$\implies$ infinite leverage. 
2. Arbitraging a claim consists of constructing a <span style="color:blue">self-financing, tradable portfolio which replicates the claim's payoff</span>. 
    - For forward contracts, the tradable portfolio is static. 
    - [Binomial branch example](#binomialBranch): in this case, pricing is possible _without knowing the underlying distribution_. 
3. The general construction of arbitrage portfolios is _dynamic_ but _self-financing. 
    - Empirical approach: we know the arbitrage price of a binomial branch; then work backward. Results in a dynamic replicating portfolio. 
    - Principled approach: use the binomial representation theorem to write the claim's martingale process $\mbb E_{\mbb Q}[X]$ in terms of the stock process under martingale measure $\mbb Q$, and use the previsible process as stock allocation. 
4. Proof of the binomial representation theorem: two branches are related by a (previsible) scale and offset; both martingales $\implies$ offset is uniformly zero, so martingales are "equivalent" up to dynamic scaling by a previsible process. 
5. High-level takeaway: arbitrary claims on a binomial tree model can be priced by arbitrage. 
    - Convergent limit of binomial process $\to$ Brownian motion with drift. 

## Teaser: forward contracts {#forwardExample -}
...
## Binomial branch model {#binomialBranch -}
...
## Binomial tree model {#binTreeModel -}
...
_Proof:_ For $j=0$ the equation holds trivially. Inductively, assuming the equation holds up to $j-1$; at time $j$, knowing the taken path and possible values of $S_j\in \{S_j^{\mrm{up}}, S_j^{\mrm{down}}$ (resp. $N_j$), construct the previsible quantity (previsible since its value does not depend on the choice of $S_j, N_j$ in their crystallizations above: 
\[ 
    \phi_j = \dfrac{N_j^{\mrm{up}} - N_j^{\mrm{down}}}{S_j^{\mrm{up}} - S_j^{\mrm{down}}}
\] 
It finally remains to determine the offset $k$ which completes the degrees of freedom in relating $N_j$ in terms of $S_j$ at the binomial branch: 
\[ 
    \Delta N_j = \phi_j \Delta S_j + k 
\] 
Take the expectation of both sides and use the martingale property 
\[ 
